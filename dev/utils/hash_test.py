...
if __name__ == "__main__":
    ...
    seed = 0xdeadbeef
    w_ids = file('my-random-numbers.txt', 'r').read().split()

    for k in range( 1, 20 ):
        print '-'*80

        lanes = dict([ (i,0) for i in range(0,k) ])
        for w in w_ids:
            h = hashlittle(w, seed)
            l = h%k
#            print '"%s": %d : %d' % (w, h, l)
...
#    file('my-random-numbers.txt', 'w').write( "\n".join([''.join(random.choice(string.ascii_uppercase + string.digits) for x in range(15)) for i in range(10000) ]) )
